...
def upload_image():
    ...
    # Set storage service NostrPic
    for x in range(10):
        if check_exists_by_xpath(navegador, '//button[@title="'+regional_settings['config_button']+'"]'):
            navegador.find_element(By.XPATH, '//button[@title="'+regional_settings['config_button']+'"]').click()
            break
        time.sleep(1)
        

    for x in range(10):
        if check_exists_by_xpath(navegador, '//div[contains(text(), "'+regional_settings['post_settings']+'")]'):
            navegador.find_element(By.XPATH, '//div[contains(text(), "'+regional_settings['post_settings']+'")]').click()
            break
        time.sleep(1)
        

    for x in range(10):
        if check_exists_by_id(navegador, 'media-upload-service-select'):
            navegador.find_element(By.ID, 'media-upload-service-select').click()
            break
        time.sleep(1)

        
    for x in range(10):
        if check_exists_by_xpath(navegador, '//span[contains(text(), "nostpic.com")]'):
            navegador.find_element(By.XPATH, '//span[contains(text(), "nostpic.com")]').click()
            time.sleep(1)
            break
        time.sleep(1)
    ...
    for x in range(10):
        ...
        if check_exists_by_xpath(navegador, '//div[contains(@role,"dialog")]//div[contains(@class,"tiptap")]/p'):
            ...
            for p in paragraph:
                time.sleep(1)
                if "https://nostpic.com/" in p.text:
                    image_link = p.text 
                    print(image_link)
                    break
            break
    
    # there is a bug with resizing dialog buttons
